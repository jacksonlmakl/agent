...
# def lightweight(prompt, model_name="TinyLlama/TinyLlama-1.1B-Chat-v1.0"):
#     # Determine device (works on both Mac and Ubuntu)
#     if torch.cuda.is_available():
#         device = "cuda"
#     elif hasattr(torch.backends, "mps") and torch.backends.mps.is_available():
...
#     # Enhance prompt with better instructions
#     enhanced_prompt = f"""You are a helpful, precise, and accurate assistant. 
# Please provide a clear, factual response to the following:
...
#     inputs = tokenizer(enhanced_prompt, return_tensors="pt", padding=True).to(device)
#     with torch.no_grad():
#         output = model.generate(
#             inputs.input_ids,
#             # max_new_tokens=150,  # Control output length
#             temperature=0.4,     # Slightly higher for more creativity while maintaining coherence
#             do_sample=False,
...
#     model_response = full_response[len(enhanced_prompt):].strip()
